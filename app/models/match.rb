class Match < ApplicationRecord
  ...
  validates_uniqueness_of :player_2, scope: %i[league_id player_1]
  ...
  scope :with_players, ->(player1, player2) do
    where('player_1=? OR player_2 = ?', player1)
      .where('player_1=? OR player_2 = ?', player2)
  end
end
